    ...
    <div class="modal-box p-0">
      <ul class="menu bg-base-200 w-full rounded-box p-4">
        ...
        <li v-else v-for="item in response">
          <span class="p-2 text-base flex justify-between">
            <span class="font-medium">{{ item.name }}</span>
            <a :href="item.link" target="_blank" rel="noopener noreferrer">
              <button class="btn w-fit btn-primary btn-outline text-white">
                Download
              </button>
            </a>
          </span>
...
onMounted(async () => {
  const { data } = await useFetch<iResponse>(
    `${API}?method=episode&session=${seriesid}&ep=${session}`,
  );
